...
-(void)addLabelWithName:(NSArray *)nameArr {
    ...
    if (SCREEN_WIDTH == 320) {  //5
        labelY = self.showYearView.frame.size.height/2 - 9 * autoSizeScaleH;
    }else if (SCREEN_WIDTH == 375 ){    //7
        labelY = self.showYearView.frame.size.height/2 - 7 * autoSizeScaleH;
    }else{  //7p
        labelY = self.showYearView.frame.size.height/2 - 6 * autoSizeScaleH;
    }
...
//    NSString *dateStr = [NSString stringWithFormat:@"%@-%@-%@ %@:%@",_yearArray[yearIndex],_monthArray[monthIndex],_dayArray[dayIndex],_hourArray[hourIndex],_minuteArray[minuteIndex]];
    ...
    if (self.dateType == DateTypeStartDate) {
        dateStr = [NSString stringWithFormat:@"%@-%@-%@ %@:%@:%@",_yearArray[yearIndex],_monthArray[monthIndex],_dayArray[dayIndex],@"00",@"00",@"01"];
    }else{
        dateStr = [NSString stringWithFormat:@"%@-%@-%@ %@:%@:%@",_yearArray[yearIndex],_monthArray[monthIndex],_dayArray[dayIndex],@"23",@"59",@"59"];
    }

    
    //self.scrollToDate = [[NSDate date:dateStr WithFormat:@"yyyy-MM-dd HH:mm"] dateWithFormatter:_dateFormatter];
    self.scrollToDate = [self transformIntoDate:dateStr];
...
- (void)setSegmenSelectIndex:(NSInteger)selectIndex {
    self.segmentView.selectedSegmentIndex = selectIndex;
